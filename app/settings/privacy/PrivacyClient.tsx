...
export default function PrivacyClient({ initialPrivacyMode }: { initialPrivacyMode: string }) {
  ...
  async function exportData() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error ?? s.couldNotExport)
      }
      ...
      setExportError(e instanceof Error ? e.message : s.couldNotExport)
    } finally {
      setExporting(false)
    }
  }
  ...
  async function deleteAccount() {
    ...
    try {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}))
        throw new Error(body.error ?? s.couldNotDelete)
      }
      const supabase = createClient()
      await supabase.auth.signOut().catch(() => {})
      router.push('/login')
    } catch (e) {
      setError(e instanceof Error ? e.message : s.couldNotDelete)
      setDeleting(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-white pb-[calc(6rem+env(safe-area-inset-bottom))]">
      <header className="px-4 pt-safe pb-3 flex items-center gap-3">
        <Link href="/settings" aria-label={s.backAria} className="text-stone-300 hover:text-white"><ArrowLeft size={22} /></Link>
        <h1 className="text-xl font-bold">{s.privacy}</h1>
      </header>
        ...
        <div className="bg-stone-900 border border-stone-800 rounded-2xl p-1.5 flex gap-1.5">
          {(['public', 'private'] as const).map(v => (
            ...
            >
              ...
              {v === 'public' ? s.publicOption : s.privateOption}
            </button>
          ))}
        </div>
        <p className="text-stone-500 text-[11px] mt-1.5 px-1">
          {vis === 'public' ? s.publicHint : s.privateHint}
        </p>
      </section>

      {/* Feed visibility — per-post sharing exists today; a global default is coming. */}
      <section className="px-4 mb-5">
        <p className="text-stone-400 text-xs uppercase tracking-wider mb-2">{s.feedVisibility}</p>
        <div className="bg-stone-900 border border-stone-800 rounded-2xl px-4 py-3 flex items-center gap-3 opacity-60">
          <span className="flex-1 text-sm text-stone-200">{s.defaultPostSharing}</span>
        ...
        <p className="text-stone-500 text-[11px] mt-1.5 px-1">{s.feedVisibilityHint}</p>
      </section>

      {/* Data */}
      <section className="px-4 mb-5">
        <p className="text-stone-400 text-xs uppercase tracking-wider mb-2">{s.data}</p>
        <button
          onClick={exportData}
          disabled={exporting}
        ...
        >
          ...
          <span className="flex-1 text-sm text-stone-200">{exporting ? s.preparingExport : s.exportMyData}</span>
      ...
      <section className="px-4">
        <p className="text-red-400/80 text-xs uppercase tracking-wider mb-2">{s.dangerZone}</p>
        <div className="bg-stone-900 border border-red-900/40 rounded-2xl p-4">
          {confirmDelete ? (
            <div className="space-y-3">
              <p className="text-stone-300 text-sm">{s.deleteConfirm}</p>
              ...
              <div className="flex items-center gap-2">
                <button onClick={deleteAccount} disabled={deleting} className="bg-red-900/70 hover:bg-red-900 text-red-100 text-sm font-semibold px-3 py-2 rounded-lg transition-colors disabled:opacity-50">
                  {deleting ? s.deletingEllip : s.deleteEverything}
                </button>
